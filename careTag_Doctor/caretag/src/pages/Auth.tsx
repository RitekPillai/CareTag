...
import { storePrivateKey, getPrivateKey,generateDoctorKeys } from '../utils/crptogrphy.ts';
...
export default function Auth() {
  ...
  // Errorsconst { error: signUpError } = await signUp(
  accountData.email,
...
const handleLogin = async (e?: React.FormEvent) => {
  ...
  setLoginError('');
  try {
    const response = await fetch('https://uncatastrophic-nonobserving-marylyn.ngrok-free.dev/doctor/login', {
      method: 'POST',
      headers: { 
    ...
    if (response.ok && data.jwtToken) {
      ...
      localStorage.setItem('refreshToken', data.responseToken);
window.location.href = "/#/";    
    } else {
      setLoginError(data.message || 'Invalid credentials or server error');
    }
  } catch (error: any) {
    setLoginError(error?.message || 'Connection error');
  } finally {
    setIsLoading(false);
  }
};
...
console.log("Generating RSA Keys...");
    const { privateKey, publicKeyString } = await generateDoctorKeys();
  ...
  try {
    ...
    const fullPayload = {
      ...
      primaryQualification: professionalData.primaryQualification,
      specialization: professionalData.specialization,
      yearsOfExperience: professionalData.yearsOfExperience,
      languagesSpoken: professionalData.languagesSpoken, 
      ...
      consultationType: practiceData.consultationType,
      publicKey: publicKeyString
    };

    console.log("Sending Payload to Backend:", fullPayload);

    const response = await fetch('https://uncatastrophic-nonobserving-marylyn.ngrok-free.dev/doctor/signup', {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        'ngrok-skip-browser-warning': 'true' 
    ...
    if (!response.ok) {
      throw new Error(resultText || `Server returned ${response.status}`);
    }
