...
public class RecordService {
  ...
  public RecordResponseAcceptDTO recordAccept(RecordRequestAcceptDTO dto) {
    ...
    return RecordResponseAcceptDTO.builder().basicDataDTO(
        BasicDataDTO.builder().fullName(patient.getFullName()).bloodGroup(patient.getBloodGroup()).dob(patient.getDob())
            .address(patient.getAddress()).careTagId(patient.getCareTagId()).gender(patient.getGender())
            .height(patient.getHeight()).weight(patient.getWeight()).allergies(patient.getAllergies())
            .image(patient.getImageUrl()).build())
        .docEmail(doctor.getEmail()).encounterId(encounter.getId()).patientId(patient.getId())
        .encrptedAesKey(dto.getAesCrptedkey()).ciphyerText(encounter.getEnvrpytedBlob()).build();
