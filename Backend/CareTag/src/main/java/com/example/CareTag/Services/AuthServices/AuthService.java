...
public class AuthService {
  ...
  // @Transactional
  // public ResponseEntity<String> doctorSignUp(SignUpRequestDTO signUpRequestDTO)
  // throws Exception {
  // User user = authUtil.getuser(signUpRequestDTO,AuthProvider.EMAIL,null,
  // RoleType.DOCTOR);
  //
  // log.info("Doctor Created: \n Email: "+user.getEmail()+" his Role
  // "+user.getRole());
  ...
  public ResponseEntity<LogResponseDTO> verifyLogin(String email, String requestOtp) {
    ...
    log.info("IS patient is already Registered " + isPatient);
  ...
  public ResponseEntity<VerifyResponse> isMyEmailVerified(String email) throws Exception {
    ...
    if (user != null) {
      ...
      return new ResponseEntity<>(new VerifyResponse(jwtToken, user.getUsername(), refreshToken.getToken()),
          HttpStatus.OK);
